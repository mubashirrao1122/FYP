/**
 * Setup SOL/USDC Pool Script
 * 
 * This script properly initializes a SOL/USDC pool with the correct ratio:
 * - 100 SOL : 10,000 USDC (= 100 USDC per SOL)
 * 
 * Usage: npx ts-node scripts/setup-sol-usdc-pool.ts
 */
...
const TARGET_USDC_AMOUNT = 10000;   // 10,000 USDC (= 100 USDC per SOL)
...
function loadTokensFromConfig(): { SOL: TokenConfig; USDC: TokenConfig } {
    ...
    if (!config.mints?.USDC) {
        throw new Error("USDC mint not found in localnet-config.json");
    }
    
    return {
        // Native SOL always uses the canonical WSOL mint address
        SOL: { mint: new PublicKey("So11111111111111111111111111111111111111112"), decimals: 9, symbol: "SOL" },
        USDC: { mint: new PublicKey(config.mints.USDC), decimals: config.decimals?.USDC ?? 6, symbol: "USDC" },
    };
}
...
// PDA derivation matching the Anchor program
function findPoolAddress(mintA: PublicKey, mintB: PublicKey): [PublicKey, number] {
    // Sort mints to match program's PDA derivation
...
async function main() {
    ...
    console.log("🏊 SOL/USDC Pool Setup");
    console.log("=".repeat(60));
    ...
    console.log(`   USDC Mint: ${tokens.USDC.mint.toBase58()}`);
    console.log(`   Target: ${TARGET_SOL_AMOUNT} SOL : ${TARGET_USDC_AMOUNT} USDC`);
    ...
    const [mintA, mintB] = tokens.SOL.mint.toBuffer().compare(tokens.USDC.mint.toBuffer()) < 0
        ? [tokens.SOL, tokens.USDC]
        : [tokens.USDC, tokens.SOL];
    
    // Determine amounts based on sorted order
    // mintA is first in sorted order, mintB is second
    const isSolFirst = mintA.mint.equals(tokens.SOL.mint);
    const amountA = isSolFirst ? TARGET_SOL_AMOUNT : TARGET_USDC_AMOUNT;
    const amountB = isSolFirst ? TARGET_USDC_AMOUNT : TARGET_SOL_AMOUNT;
    ...
    // Derive PDAs
    const [poolPda, poolBump] = findPoolAddress(mintA.mint, mintB.mint);
    ...
    if (existingPool) {
        ...
        const currentRatio = (Number(existingPool.reserveA) / Math.pow(10, mintA.decimals)) / 
                           (Number(existingPool.reserveB) / Math.pow(10, mintB.decimals));
        console.log(`   Current ratio: ${currentRatio.toFixed(4)} ${mintA.symbol}/${mintB.symbol}`);
    ...
    if (balanceA < neededA) {
        if (mintA.mint.equals(NATIVE_MINT)) {
            // WSOL: transfer SOL into the ATA then syncNative
            ...
            wrapTx.sign(wallet);
            const wrapSigA = await connection.sendRawTransaction(wrapTx.serialize(), { skipPreflight: false });
            await connection.confirmTransaction(wrapSigA, "confirmed");
        } else {
            console.log(`   Minting ${amountA} ${mintA.symbol}...`);
            await mintTo(
                connection, wallet, mintA.mint, userTokenA.address, wallet,
                BigInt(neededA)
            );
        }
    }
    ...
    if (balanceB < neededB) {
        if (mintB.mint.equals(NATIVE_MINT)) {
            ...
            await mintTo(
                connection, wallet, mintB.mint, userTokenB.address, wallet,
                BigInt(neededB)
            );
        }
    }
